...
1. Create the entity class under `android/app/src/main/java/com/acfharbinger/mobilefortress/engine/entities/`, implementing the same `update(deltaMs: Float)` / `render(canvas: Canvas, paint: Paint)` contract as `Ball.kt`.
...
4. Add a unit test under `android/app/src/test/java/com/acfharbinger/mobilefortress/` asserting the entity's update math (e.g. position after N steps, collision response).
