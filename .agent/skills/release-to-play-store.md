...
4. Locally: `just assemble-release` produces a signed `.aab` under `android/app/build/outputs/bundle/release/`. Sanity-install the equivalent APK (`./gradlew assembleRelease`) on a device to smoke-test the release build specifically (R8 minification can behave differently from debug).
