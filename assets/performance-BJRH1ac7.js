...
| # | Item | Effort | Status |
| --- | --- | --- | --- |
| P1 | ~~Fixed-timestep \`GameLoop\` with capped catch-up iterations~~ — template baseline, carried forward into the C++ core's tick loop | S | ✅ Done |
| P2 | Migrate simulation state to an ECS (EnTT) inside the shared C++ core for cache-local, sparse-set-based iteration over hundreds of siege entities | L | 📋 Pending |
...
| P4 | Entity/component pooling to avoid per-frame allocation in the update/render hot path (both native clients and the C++ core) | M | 📋 Pending |
| P5 | Zero-copy state snapshots (FlatBuffers) across the JNI/Swift-C++-interop boundary to avoid per-frame (de)serialization cost | M | 📋 Pending |
