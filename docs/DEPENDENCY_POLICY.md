...
| Node.js | `docs/website` | 20 LTS | Matches `actions/setup-node@v4`'s `node-version` in `.github/workflows/docs.yml` |
| npm | `docs/website` | 10+ (ships with Node 20) | `package-lock.json` v3 lockfile format |
...
- The root `package.json` declares `docs/website` as an npm workspace — `npm install`/`npm ci` at the **repo root** resolves and hoists dependencies for every workspace member into a single root `node_modules/`, with one root `package-lock.json` as the single lockfile (`docs/website` has no lockfile of its own).
...
- `docs/website/package.json` uses `^` (caret) ranges for all dependencies — Vue, Vue Router, `markdown-it` and its plugins, `highlight.js`, `mermaid`, and KaTeX are all still pre-1.0-breakage-sensitive enough that patch/minor bumps should go through the normal `npm update` + test cycle rather than being pinned exact.
...
| **Minor version** (new features, backwards-compatible) | As Dependabot proposes | `.github/dependabot.yml` opens PRs automatically for Gradle, npm (`docs/website`), and GitHub Actions — review and merge rather than bumping by hand. |
...
1. **Is it already available?** `docs/website` already covers Markdown rendering (`markdown-it`), syntax highlighting (`highlight.js`), diagrams (`mermaid`), and math (`katex`) — check before reaching for an alternative that duplicates one of these.
2. **What is the maintenance status?** Prefer packages with a release in the last 12 months and a responsive issue tracker.
3. **What is the transitive footprint?** For Android/Kotlin: check the dependency's own transitive graph doesn't pull in a second copy of Kotlin coroutines or a conflicting AndroidX version. For `docs/website`: run `npm ls <package>` after installing to inspect what it pulled in.
...
Once approved: add to the correct manifest (`gradle/libs.versions.toml`, `docs/website/package.json`, or the relevant `infra/` module), regenerate the lockfile, and document the addition in [`docs/moon/CHANGELOG.md`](moon/CHANGELOG.md).

---

## Removing a Dependency

1. Verify no code paths still reference it (`grep -r` across `android/app/src/`, `ios/MyGame/`, or `docs/website/src/` as applicable).
2. Remove it from the manifest and regenerate the lockfile (`./gradlew --refresh-dependencies`, or `npm install` at the repo root).
3. Run the relevant test suite (`just unit-test`, `just ios-test`, or `docs/website`'s `npm run build`).
...
- `.github/dependabot.yml` covers Gradle, npm (`docs/website`), and GitHub Actions version bumps, including security advisories.
- For `docs/website`, run `npm audit --audit-level=moderate` locally before a release-adjacent change.
