...
- Combined the interactive design-hub website (`docs/design/website/`) and the MkDocs reference documentation into a single site under `docs/public/`: `docs/public/{index.html,styles.css,script.js,assets/}` is the design-hub landing page (now with a "Documentation" nav link), and `docs/public/docs/` is populated at deploy time with the MkDocs build output. Added `docs/design/`/`docs/moon/*` pages to `docs/mkdocs.yml`'s nav (Roadmap and Design sections) and excluded `docs/public/` from the MkDocs build via `exclude_docs`.
- Reworked `.github/workflows/docs.yml` to build the combined `docs/public/` site and publish it to a `gh-pages` branch via `peaceiris/actions-gh-pages`, instead of GitHub's managed Pages artifact/OIDC deploy. Runs `mkdocs build` without `--strict` since several docs intentionally link outside `docs_dir` (e.g. to `.agent/`, `android/`, `ios/`, `reports/`, `research/`) for GitHub browsing.
