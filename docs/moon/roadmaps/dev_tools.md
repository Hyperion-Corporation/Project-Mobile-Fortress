...
6. **DT6 last** — unblocked by T35 (two dual-front JSONs); overlay picker is T37.
...
| DT6 | Level/scenario picker: jump straight to any level or wave setup from a dev menu instead of always starting `slice0_dual_front` | S | 🚧 **Slice-0 wired (T37)** — overlay `LevelPickSelect` + in-place `debug_load_level` | Player `LevelSelect` is G5. Wave jump stays DT3. |
