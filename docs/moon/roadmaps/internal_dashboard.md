...
| --- | --- | --- | --- | --- | --- |
| ID1 | Define dashboard requirements: which game/product metrics matter first (retention, monetization, telemetry, lore/product tracking, player feedback) — scope before building | S | — | 🚧 In Progress — `DashboardRequirementsView.tsx` shipped (T14, Gemini, 2026-08-14); metric priority table (P0/P1/P2), run history schema, and scope/sequence tracker committed | #121 |
| ID2 | Information architecture & wireframes for the dashboard surface within the existing React host | M | ID1 | ✅ Delivered — `docs/website/DASHBOARD_IA.md` committed (Gemini, 2026-08-14): route map, 4 ASCII wireframes, static data schemas, ID3 guidance | #122 |
| ID3 | Stand up the dashboard website skeleton as a new `docs/website/src/frameworks/react` view (or a dedicated island, per the Authoring guide below) | M | ID2, MFP1–MFP3 | ✅ Delivered — Native React dashboard (`DashboardView.tsx`, `RunHistoryView.tsx`, `CiStatusView.tsx`, `PlaytestNotesView.tsx`, `useDashboardData.ts`, vitest suite) | #123 |
...
| ID6 | Zoomable in-game-lore map: drill into a building/settlement/coastal outpost for local lore text, zoom out for regional/historical context — reuses `CoastalFlowField.astro`'s coordinate model (MFP5) as a starting point, not a from-scratch map | L | MFP5 | ✅ Delivered — Interactive zoomable coastal lore map (`LoreMapView.tsx`) with 3 zoom levels, flow vectors, raid corridors, and outpost lore drill-down | — |
| ID7 | 3D model viewer + data-visualization component library for the dashboard (unit/hero 3D previews, dynamic + interactive charts/plots for telemetry and monetization) — evaluate against [`Vue 3 Visualization Stack Research.md`](../research/Vue%203%20Visualization%20Stack%20Research.md)'s library findings, re-scoped to the current React host | L | ID3 | 🚧 Partial — `UnitVisualizerView.tsx` delivers an interactive 2.5D SVG tactical inspector (rotation, action states, palettes, combat metrics); a true 3D renderer and reusable dynamic chart/plot library remain open | — |
