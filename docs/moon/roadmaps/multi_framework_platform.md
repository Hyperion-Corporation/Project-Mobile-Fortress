...
| MFP16 | Authoring guide: when host vs foreign-root island vs WASM; teardown rules; ADR links | M | MFP7, MFP11, MFP15 | ✅ [`multi_framework_authoring_guide.md`](multi_framework_authoring_guide.md) |
...
| 2026-08-09 | R4 | Delivered MFP16: [`multi_framework_authoring_guide.md`](multi_framework_authoring_guide.md) — the decision guide for native React component vs foreign-root island (Aurelia pattern) vs iframe island (Astro pattern) vs WASM (forward-looking), teardown rules, and links into `APP.md`'s per-island rationale. Wired into `docs/mkdocs.yml`'s Roadmap nav alongside this file. |
