...
| U9 | Art/asset polish (sequenced sub-passes, see below) | M | ✅ **Delivered — 2026-08-15** — `unit_token.gd` procedural silhouettes for full roster + hero badges + dynamic outpost health tiers + 6-tile terrain atlas (`iso_tiles.png` coast/ocean/path/marsh/shoal/bastion) with ukiyo-e wave foam & elevation contours | Sub-passes: **(1)** silhouette/readability on the current roster — spearman, cannon, arquebusier, junk, Qi, Dias, cross-support, land/sea raider (VS-A3-relevant, can precede VS10); **(2)** HQ/outpost visual tiers driven **only** by state that already exists in the sim — G7 HP ratio (full/damaged/lost) — no promised "upgraded building" tier until a building-upgrade sim flag exists (`upgrade_defender` is unit-only today); **(3)** tile-atlas variety (coast/marsh/shoal) last. Silhouette pass may land before VS10 if current art fails VS-A3; tiles and full illustrated polish are Phase-1 follow-up, not a gate. |
