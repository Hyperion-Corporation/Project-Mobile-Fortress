...
\`SimulationCore.save_state\`/\`load_state\` uses FlatBuffers schema \`game/src/schema/simulation_state.fbs\`. Modular battle S/L + auto-snapshot on run end. JNI/Swift boundary for this issue remains future; Godot GDExtension path is live for desktop/export packaging."

gh issue comment 63 --body "## Progress 2026-08-11 — iOS export packaging

iOS preset (min 17.0) in \`game/export_presets.cfg\`. Android debug APK export verified on Linux. iOS IPA still needs macOS/Xcode. GDExtension \`ios.arm64\` path deferred until dylib build."
